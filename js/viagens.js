...
// Função para exibir os detalhes da viagem no modal
async function mostrarDetalhesViagem(   id) {
    try {
        // Chama a função getViagem para obter os dados da viagem
        const viagem = await getViagem(id);  // A função getViagem agora recebe 'id'

        // Preenche os detalhes da viagem no modal
        const detalhesViagem = document.getElementById('detalhesViagem');
        detalhesViagem.innerHTML = `
            <p><strong>ID Viagem:</strong> <input type="text" id="id_viagem" value="${viagem.id}" disabled></p>
            ...
            <p><strong>Horário de Partida:</strong> <input type="time" id="horario_partida" value="${formatarHorario(viagem.horario_partida)}" disabled></p>
            ...
            <p><strong>Empresa:</strong> ${viagem.empresa_nome || 'Não encontrado'}</p>
        `;
